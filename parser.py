...
class Parser(object):
    ...
    def __init__(self, model=None, oracle_type=DETERMINE_TREE_TO_GRAPH_ORACLE_SC,
    ...
    @staticmethod
    def get_best_act(scores, actions):
    ...
    @staticmethod
    def get_best_act_constraint(scores, actions, argset):
        best_label_index = None
        best_act_ind = np.argmax(map(np.amax, scores))
        if actions[best_act_ind]['type'] in ACTION_WITH_EDGE:
            ...
            while best_label_index in argset:
                ...
                best_act_ind = np.argmax(map(np.amax, scores))
                if actions[best_act_ind]['type'] in ACTION_WITH_EDGE \
                        or actions[best_act_ind]['type'] in ACTION_WITH_TAG:
                    best_label_index = scores[best_act_ind].argmax()
                else:
                    best_label_index = None
    ...
    def parse_corpus_train(self, instances, interval=500):
        ...
        print >> self.elog, "One pass on %s " \
                            "instances takes %s" % (i, datetime.timedelta(seconds=round(time.time()-start_time, 0)))
            ...
            if len(actions) == 1:
                ...
                if train:
                    ...
                    for feature, action in zip(features, actions):
                        scores.append(state.get_score(action['type'], feature, True))
                    ...
                    try:
                        ...
                        if self.verbose > 2:
                            print >> sys.stderr, 'WARNING: gold action %s ' \
                                                 'not in possible action set %s' % (str(gold_act), str(actions))
                    ...
                    if self.verbose > 2:
                        print >> sys.stderr, "Step %s: (%s,%s) | " \
                                             "take action %s, label:%s | " \
                                             "gold action %s,label:%s | " \
                                             "State:sigma:%s beta:%s" % (
                            step, actions[best_act_ind]['type'], gold_act['type'], actions[best_act_ind],
                            best_label, gold_act, gold_label, state.sigma, state.beta)

                    if self.verbose > 3:
                        # correct next2 tag error
                        if gold_act['type'] == NEXT2:
                            self.output_weight(best_act_ind, best_label_index, features, actions)
                            if gold_act.get('tag', None) != best_act.get('tag', None) and \
                                    'tag' in gold_act and not (isinstance(gold_act['tag'], (ETag, ConstTag)) or
                                                               re.match('\w+-\d+', gold_act['tag'])):
        ...
        while False:
            ...
            for prev in beams[-1]:
                for action in prev.get_possible_actions():
                    pass
        raise NotImplemented('Beam search is not implemented.')
    ...
    def errorAnalyze(self, parsed_span_graph, gold_span_graph, instance, error_stat):
        ...
        state = Parser.State.init_state(instance, self.verbose)
        seq = []
        for r in sorted(parsed_span_graph.multi_roots, reverse=True):
            seq += parsed_span_graph.postorder(root=r)
        seq.append(-1)
        sigma = Buffer(seq)        
        sigma.push(START_ID)
        state.sigma = sigma
        ...
        while not state.is_terminal():
            ...
            if state.is_permissible(action):
                if action['type'] == NEXT1:
                    if label is not None and label != START_EDGE:
